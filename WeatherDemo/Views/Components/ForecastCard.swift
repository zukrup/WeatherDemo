    ...
    var body: some View {
        ZStack {
            ...
            RoundedRectangle(cornerRadius: 30)
                ...
                .overlay {
                    // MARK: border
                    RoundedRectangle(cornerRadius: 30).strokeBorder(Color.white.opacity(isActive ? 0.5 : 0.2)).blendMode(.overlay)
                }
            ...
            VStack(spacing: 16) {
                ...
                VStack(spacing: -4, content: {
                    ...
                    Text(forecast.probability, format: .percent).font(.footnote.weight(.semibold))
                        .foregroundColor(Color.probabilityText).opacity(forecast.probability > 0 ? 1 : 0)
                })
                .frame(height: 42)
